...
# AUC comparison
weighted_lasso_pred <- predict(weighted_lasso, newx = x, s = "lambda.1se", type = "response")
...
pred_w_glm <- predict(weighted_glm, type = "response")
auc_w_glm <- get_auc(y, pred_w_glm)
pred_u_glm <- predict(unweighted_glm, type = "response")
auc_u_glm <- get_auc(y, pred_u_glm)

pred_w_lasso <- predict(weighted_lasso, newx = x, s = "lambda.1se", type = "response")
auc_w_lasso <- get_auc(y, pred_w_lasso)
pred_u_lasso <- predict(unweighted_lasso, newx = x, s = "lambda.1se", type = "response")
auc_u_lasso <- get_auc(y, pred_u_lasso)

pred_w_gam <- predict(weighted_gam, type = "response")
auc_w_gam <- get_auc(y, pred_w_gam)
pred_u_gam <- predict(unweighted_gam, type = "response")
auc_u_gam <- get_auc(y, pred_u_gam)
